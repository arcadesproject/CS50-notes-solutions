...
bool check(char *word)
{
    ...
    for(int i = 0; i < length; i++)
    {
        ...
        // Map character to index (assuming 'a' to 'z' → 0 to 25)
        int ch_index = ch - 'a';

        // If the corresponding child node doesn't exist, word isn't in trie
        if (cursor->children[ch_index] == NULL)
        {
            return false;
        }
        else
        {
            // Move cursor to the child node to continue traversal
            cursor = cursor->children[ch_index];
        }
    }
